...
def get_framework_adapter(model: Any, config: dict) -> Any:
    """Dynamically infers the framework based on the model type or configuration.
    ...
    # Check if it's a PyTorch model or requested via config
    if "torch" in model_type_str or backend == 'pytorch':
        try:
    ...
    # Check if it's a Keras or TensorFlow model or requested via config
    elif "keras" in model_type_str or "tensorflow" in model_type_str or backend == 'keras':
        try:
    ...
    raise UnsupportedFrameworkError(f"Could not infer framework for model of type {type(model)} and backend '{backend}'")
