...
def main():
    ...
    config = {
        'model_type': 'resnet18',
        'backend': 'pytorch',
        'method': 'mean_abs_act', # Testing the newly fixed Activation method!
        'scope': 'global',        # Global scope allows varying reduction across layers
        'ratio': 0.5,             # Remove 50% of filters globally
        'epochs': 0,              # No baseline training needed, we load pretrained
        'ft_epochs': 1,           # Quick fine-tuning for demonstration
        'pretrained_checkpoint_path': './checkpoints/resnet18_baseline.pth',
        'pruned_checkpoint_path': './checkpoints/resnet18_pruned_untrained.pth',
        'final_checkpoint_path': './checkpoints/resnet18_pruned_finetuned.pth'
    }
    ...
    # In reality, the user provides their own checkpoint.
    os.makedirs('./checkpoints', exist_ok=True)
    if not os.path.exists(config['pretrained_checkpoint_path']):
        print("Creating a dummy pretrained model checkpoint to simulate user input...")
        from reducnn.backends.torch_backend import PyTorchAdapter
        adapter = PyTorchAdapter(config)
        dummy_model = adapter.get_model('resnet18', input_shape=(3, 32, 32), num_classes=10)
    ...
    fig = animator.plot_architecture(pruned_model, title="Pruned ResNet18 Architecture", render=False)
    
    # Export it to an HTML file locally
    html_path = animator.export_html(fig, path="pruned_resnet_architecture.html")
